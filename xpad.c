...
static int trigger_rumble_mode = 1;
module_param(trigger_rumble_mode, int, S_IWUSR | S_IRUGO);
MODULE_PARM_DESC(trigger_rumble_mode, "Impulse trigger rumble mode: 0=off, 1=mirror, 2=pressure, 3=gc_click (default 1)");
...
static const struct xpad_device {
	...
	{ 0x2dc8, 0x2030, "8BitDo Ultimate 3-mode Controller for Xbox (rev 2)", MAP_SHARE_BUTTON, XTYPE_XBOXONE }, /* My version of this controller had a different ID */
...
#define XPAD_OUT_CLICK_IDX	(1 + IS_ENABLED(CONFIG_JOYSTICK_XPAD_FF) + \
				 IS_ENABLED(CONFIG_JOYSTICK_XPAD_LEDS))
#define XPAD_NUM_OUT_PACKETS	(2 + \
...
struct usb_xpad {
	...
	u16 ltrig_pos;			/* last trigger position for rumble, 0-1023 */
	u16 rtrig_pos;
	bool ltrig_clicked;		/* gc_click edge state */
	bool rtrig_clicked;
...
static void xpad_send_trigger_click(struct usb_xpad *xpad, bool left, u8 magnitude);
...
static void xpadone_process_packet(struct usb_xpad *xpad, u16 cmd, unsigned char *data, u32 len)
{
	...
	if (data[0] == GIP_CMD_VIRTUAL_KEY) {
		...
		if (xpad->mapping & MAP_TRIGGERS_TO_BUTTONS) {
			...
			xpad->ltrig_pos = (__u16) le16_to_cpup((__le16 *)(data + 6));
			xpad->rtrig_pos = (__u16) le16_to_cpup((__le16 *)(data + 8));
			input_report_abs(dev, ABS_Z, xpad->ltrig_pos);
			input_report_abs(dev, ABS_RZ, xpad->rtrig_pos);
		}
...
/* The funny for the GC trigger emu */
static void xpad_send_trigger_click(struct usb_xpad *xpad, bool left, u8 magnitude)
{
	struct xpad_output_packet *packet =
			&xpad->out_packets[XPAD_OUT_CLICK_IDX];
	unsigned long flags;
	...
	packet->data[4] = 0x00;
	packet->data[5] = left ? GIP_MOTOR_LT : GIP_MOTOR_RT;
	packet->data[6] = left ? magnitude : 0x00;
...
static int xpad_play_effect(struct input_dev *dev, void *data, struct ff_effect *effect)
{
	...
	case XTYPE_XBOXONE:
		...
		switch (trigger_rumble_mode) {
		case 1: /* mirror */
			packet->data[6] = strong / 1024;
			packet->data[7] = weak / 1024;
			break;
		case 2: /* pressure based */
			packet->data[6] = (strong / 1024) * xpad->ltrig_pos / 1023;
			packet->data[7] = (weak / 1024) * xpad->rtrig_pos / 1023;
			break;
		case 3: /* gc_click: rumble forces triggers to 0 (avoid latched clicks) */
			packet->data[5] = GIP_MOTOR_ALL;
			packet->data[6] = 0x00;
			packet->data[7] = 0x00;
			break;
